...
const CreatorCard: FC<{ item: CreatorType, index: number }> = ({ item, index }) => {
    return (
        <>
            <div className='hidden lg:block w-[240px] relative bg-[#3B3B3B] text-center rounded-[20px] p-[20px]'>
            ...
            <div className='w-[315px] lg:hidden'>
                <div className='flex gap-[20px] relative bg-[#3B3B3B] text-start rounded-[20px] p-[20px]'>
                    <CustomImage src={`${API}/file/${item.image}`} alt='creator img' W={60} H={60} />
                    <div>
                        <Heading classList='!mb-[5px]' tag='h3'>{item.username}</Heading>
                        <Text><span className='text-[#858584]'>Total Sales:</span> 34.53 ETH</Text>
                    </div>
                    <button className='w-[30px] h-[30px] bg-[#2B2B2B] rounded-full text-[16px] font-normal text-[#858584] absolute top-[13px] left-[12px] flex items-center justify-center'>{index + 1}</button>
                </div>
            </div>
        </>
    )
